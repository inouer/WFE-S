...
def uploadhtml(url, host, charset, useragent, password, html_data):
    #ホスト名
...
#         print soup.body
    ...
    for script in script_tags:
        ...
        if script_inner == "":
            #srcの取り出し
            r3 = re.compile('<script.*?src=[\"\'](.*?)[\"\'][^>]*>.*?</script>')
            ...
            try:
                ...
                if r.search(target_script) is not None:
                    ...
                    r = re.compile('//.*?')
                    if r.search(target_script) is not None:
                        fp = opener.open("http:"+target_script)
                    else:                    
        ...
        js_entity.original_url = target_script
        js_entity.put()
...
#                     script.extract()
        ...
        js_id_list.append(js_id)
    
    #scriptタグを追加
    js_id_list.reverse()
    for js_id in js_id_list:
...
#        soup.findAll("head")[0].insert(0,svg_js)
    ...
    # 変更結果のHTML
    html_result = unicode(soup.findAll("html")[0]).encode(charset,'ignore')
...
#        self.response.out.write(template.render(path, template_values))
    ...
    def get(self):        
        ...
        page_id = uploadhtml(url, host, charset, useragent, password, html_data)
...
class UploadFullHTML(webapp.RequestHandler):
    def get(self):
        url = self.request.GET.get("url").encode('utf8')
        host = self.request.GET.get("host").encode('utf8')
        charset = self.request.GET.get("charset").encode('utf8')
        ...
        html_data = self.request.GET.get("html").encode('utf8')
